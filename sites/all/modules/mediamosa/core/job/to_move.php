 ...
 */
function _vpx_jobs_create_new_job_transcode($job_id, $a_args) {
  $a_parameters = array(
    'job_id' => array(
      'value' => $job_id,
      'type' => 'int',
    ...
    'profile_id' => array(
      'value' => vpx_get_parameter_2($a_args['post'], 'profile_id'),
      'type' => 'int'
    ),
    'tool' => array(
  ...
  if (($a_parameters['profile_id']['value'] == "") && ($a_parameters['tool']['value'] == "")) {
    db_set_active();
    $db_result = db_query("
        SELECT transcode_profile_id, profile, tool, file_extension, command
        FROM {transcode_profile}
        WHERE is_default_profile = 'TRUE'");
    if ($db_result_row = db_fetch_object($db_result)) {
      $profile_id = $db_result_row->transcode_profile_id;
      ...
      return new rest_response(vpx_return_error(ERRORCODE_NO_DEFAULT_TRANSCODE_PROFILE));
    }
  }
...
/**********************************************************************
 * Zetten van de eigenschappen van een job
 *********************************************************************/

/**
 * Zet de status van een job op basis van een job_id
 */
function vpx_jobs_set_job_status($a_args) {
  ...
  $a_parameters = array(
    ...
    'status' => array(
      'value' => vpx_get_parameter_2($a_args['post'], 'status'),
      'type' => 'status_type',
      'custom_error' => ERRORCODE_WRITING_UNKNOWN_JOBSTATUS
    ),
    'progress' => array(
      'value' => vpx_get_parameter_2($a_args['post'], 'progress', "novalue"),
      'type' => 'skip'
    ),
    'error_description' => array(
      'value' => vpx_get_parameter_2($a_args['post'], 'error_description', ""),
      'type' => 'skip'
    ),
    // Stills
    'create_still' => array(
  ...
  $jobinfo = _vpx_jobs_get_job_info($a_parameters['job_id']['value']);
  $mediafile_id = $jobinfo['mediafile_id'];

  // wanneer een upload job op finished wordt gezet, start een analyse job
  if (($jobinfo['job_type'] == JOBTYPE_UPLOAD) &&
      ($a_parameters['progress']['value'] == "1.000") &&
      ($jobinfo['status'] != JOBSTATUS_FINISHED)) {

        // controleer of de upload echt compleet is
      db_set_active('data');
      $s_sannas_mount_point = db_result(db_query("SELECT sannas_mount_point FROM {mediafile} WHERE mediafile_id='%s'", $mediafile_id));

      clearstatcache();
      $current_filesize = filesize($s_sannas_mount_point . DS . DATA_LOCATION . DS . $mediafile_id{0} . DS . $mediafile_id);
      $db_filesize = db_result(db_query("SELECT file_size FROM {upload_job} WHERE job_id = %d", $a_parameters['job_id']['value']));
      db_set_active();

      if ($db_filesize == $current_filesize) {
...
/**
 * ophalen van informatie van 1 job
 */
function _vpx_jobs_get_job_info($job_id) {
  ...
  $db_result = db_query(
     "SELECT jb1.*,
            tj1.transcode_profile_id, tj1.tool, tj1.command, tj1.file_extension,
            sj1.size, sj1.frametime,
            uj1.file_size, uj1.uploaded_file_size
      FROM job jb1
      LEFT OUTER JOIN transcode_job tj1 ON tj1.job_id = jb1.job_id
      LEFT OUTER JOIN still_job sj1 ON sj1.job_id = jb1.job_id
      LEFT OUTER JOIN upload_job uj1 ON uj1.job_id = jb1.job_id
      WHERE jb1.job_id = %d", $job_id);
  db_set_active();

  // haal voor de volgende info de gegevens uit de resultset en stop die
  // in een array
  $fields = array('job_id', 'asset_id', 'mediafile_id', 'owner', 'app_id',
                  'status', 'progress', 'priority', 'job_type',
                  'started', 'finished', 'testtag', 'error_description',
                  'created', 'changed', 'transcode_profile_id', 'tool',
                  'command', 'file_extension', 'size', 'frametime',
                  'file_size', 'uploaded_file_size');
  if ($db_result_row = db_fetch_array($db_result)) {
    foreach ($fields as $field) {
      $jobinfo[$field] = $db_result_row[$field];
    }
  }
  return $jobinfo;
}
...
function vpx_jobs_get_analysis_upload($app_id, $mediafile_src) {
  assert($app_id);
  ...
  // Remove old metadata
  db_set_active("data");
  db_query("DELETE FROM {mediafile_metadata} WHERE mediafile_id = '%s'", $mediafile_src);
  db_set_active();
  ...
  watchdog('server', 'Analyse string: '. print_r($execution_string, TRUE));
  $a_output = array();
...
/**
 * Haal de status op van alle jobs die aan de server zijn gekoppeld.
 * Wanneer een job de status FINISHED of FAILED heeft, dan kan de
 * jobhandler de 'remove_job' call maken om deze uit de lijst te verwijderen.
 */
function vpx_jobserver_get_jobs_status($a_args) {
  ...
  if ($a_parameters['testtag']['value'] == '') {
    $a_parameters['testtag']['value'] = 'FALSE';
  }
  ...
  // Haal de lijst met jobs uit de database
  db_set_active("jobserver");
  $query_result = db_query("SELECT job_id, status, progress, job_type, mediafile_dest,
     UNIX_TIMESTAMP(started) AS started, UNIX_TIMESTAMP(finished)
     AS finished, analyse_result, error_description
     FROM {jobserver_job} sj1
     LEFT OUTER JOIN {jobserver_analyse_job} saj1
     ON sj1.jobserver_job_id = saj1.jobserver_job_id
     WHERE testtag = '%s'", $a_parameters['testtag']['value']);
  db_set_active();

  while ($query_result_row = db_fetch_object($query_result)) {
    $job = array();
    $job["job_id"] = $query_result_row->job_id;
    $job["status"] = $query_result_row->status;
    $job["progress"] = $query_result_row->progress;
    $job["job_type"] = $query_result_row->job_type;
    $job["mediafile_dest"] = $query_result_row->mediafile_dest;
    $job["started"] = ($query_result_row->started == null) ? "" : date("d-m-Y H:i", $query_result_row->started);
    $job["started_unix"] = ($query_result_row->started == null) ? "" : $query_result_row->started;
    $job["finished"] = ($query_result_row->finished == null) ? "" : date("d-m-Y H:i", $query_result_row->finished);;
    $job["finished_unix"] = ($query_result_row->finished == null) ? "" : $query_result_row->finished;
    $job["analyse_result"] = $query_result_row->analyse_result;
    $job["error_description"] = $query_result_row->error_description;
    $rest_jobs->add_item($job);
  }
  ...
  // Voeg de status van de request samen met de lijst van jobs
    ...
    return $rest_jobs;
} // end of vpx_jobserver_get_jobs_status
